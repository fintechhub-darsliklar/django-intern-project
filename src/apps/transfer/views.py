...
import json
from apps.send_sms import send_telegram_otp
...
def is_valid_transfer(transfer):
    ...
    # created_at va hozirgi vaqt orasidagi farqni hisoblaymiz
    # Agar farq 1 daqiqadan kichik bo'lsa True qaytaradi
    if now - transfer.created_at <= timedelta(minutes=1):
        return True
...
def transfer_create(ext_id, sender_card_number, sender_card_expiry,
    ...
    if Transfer.objects.filter(ext_id=ext_id).exists():
        ...
        data = {"error": {"code": error.code, "message": error.en}}
        return Error(data)
    ...
    otp_sent = send_telegram_otp(sender_card.phone, otp)
